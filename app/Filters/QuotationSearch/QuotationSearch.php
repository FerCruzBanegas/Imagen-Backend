...
class QuotationSearch extends Search
{
	public static function checkSortFilter(Request $request, Builder $query)
    {
        if ($request->filled('sort')) {
            $sort = $request->input('sort');
            return $query->orderBy($sort[0]['field'], $sort[0]['dir'])->checklist()->relations();
              
        } else {
            return $query->orderBy('id', 'DESC')->checklist()->relations();
        }
    }
    ...
    {
        return $query->orderBy('id', 'DESC')->checklist()->relations()->paginate($request->take);
    }
}